...
func _on_body_entered(body: Node2D) -> void:
	if UI.gas < 90 and UI.money_for_gas(gas_added,gas_add_rate):
		UI.gas = snapped(UI.gas,5.0)
		gas_click.pitch_scale = (UI.gas/100) +.5
		prints("starting",body)
		gas_timer.start()
		UI.get_gas()


func _on_gas_guzzle_timeout() -> void:
	if not UI.money_for_gas(gas_added,gas_add_rate):
		prints("Not Enough Money!",UI.gas)
		...
		gas_timer.wait_time = lerp(gas_timer.wait_time,.1,.15)
